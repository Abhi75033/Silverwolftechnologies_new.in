...
export function generateFallbackAreaData(city: string, subCity: string, serviceTitle: string) {
  ...
  // ── Intro ─────────────────────────────────────────────────────────────────
  const intro = fill(INTROS[hash % INTROS.length], vars);

  // ── H2 headings ──────────────────────────────────────────────────────────
  const h2Set = H2_SETS[hash % H2_SETS.length];
  ...
  } else if (/crm|saas|web app/.test(svcLower)) {
    faqCandidateIndices = [5, 10, 11, 9]; // SaaS, technical, ROI, maintenance
  ...
  const faqSet = FAQ_SETS[faqSetIndex];
