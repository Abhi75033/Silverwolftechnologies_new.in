...
export const Contact = ({ compact = false, presetService }: Props) => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const text = [
      `🐺 *New Enquiry — Silver Wolf Technologies*`,
      ``,
      `👤 *Name:* ${name}`,
      `📧 *Email:* ${email}`,
      `📞 *Phone:* ${phone || "Not provided"}`,
      presetService ? `🛠️ *Service:* ${presetService}` : null,
      ``,
      `📝 *Message:*`,
      message,
    ].filter(Boolean).join("\n");
    ...
    toast.success("Opening WhatsApp — please tap Send to complete your enquiry!");
        ...
        <div className="grid lg:grid-cols-5 gap-10 items-start">
          ...
          <form onSubmit={onSubmit} className="lg:col-span-3 glass rounded-2xl p-8 space-y-5 animate-fade-up">
            ...
            <Button type="submit" variant="hero" size="lg" disabled={loading} className="w-full">
              {loading ? "Opening WhatsApp..." : <><Send className="h-4 w-4 mr-2" />Send via WhatsApp</>}
            </Button>
